...
const POApprovalListLayer = () => {
  ...
  const filteredAndSortedPOs = useMemo(() => {
    let filtered = poData.filter(po => {
      ...
      if (dateRangeFilter.start) {
        matchesDateRange = matchesDateRange && new Date(po.poDate) >= new Date(dateRangeFilter.start);
      }
      if (dateRangeFilter.end) {
        matchesDateRange = matchesDateRange && new Date(po.poDate) <= new Date(dateRangeFilter.end);
      }
    ...
    return filtered;
  }, [poData, searchTerm, statusFilter, priorityFilter, departmentFilter, dateRangeFilter, sortBy, sortOrder]);
  ...
  const handleApprovePO = async (po) => {
    try {
      ...
      if (response.success) {
        // Update local state
        setPoData(prev => prev.map(p => p.id === po.id ? response.data : p));
    ...
    try {
      ...
      if (response.success) {
        ...
        response.data.forEach(updatedPO => {
          setPoData(prev => prev.map(p => p.id === updatedPO.id ? updatedPO : p));
        });
        setSelectedPOs([]);
    ...
    <div className="card">
      ...
      {showBulkActions && (
        <div className="alert alert-info d-flex align-items-center justify-content-between mb-0">
          <div className="d-flex align-items-center gap-2">
            ...
            <span>{selectedPOs.length} PO{selectedPOs.length > 1 ? 's' : ''} selected</span>
          </div>
      ...
      <div className="card-body">
          ...
          <table className="table bordered-table mb-0">
              ...
              <tr>
                ...
                >
                  <div className="d-flex align-items-center gap-1">
                    PO No
                    {sortBy === 'poNo' && (
                      <Icon icon={`mdi:arrow-${sortOrder === 'asc' ? 'up' : 'down'}`} />
                    )}
                  </div>
                </th>
                <th 
            ...
            <tbody>
              {loading ? (
                ...
                paginatedPOs.map((po) => (
                  <tr key={po.id} className={selectedPOs.includes(po.id) ? 'table-active' : ''}>
                    <td>
                      <div className="form-check style-check d-flex align-items-center">
                        <input
                          className="form-check-input"
                    ...
                    <td>
                      <div className="d-flex align-items-center gap-2">
                        <div className="flex-grow-1">
                          ...
                          <small className="text-muted">{po.supplier.code}</small>
                        </div>
                      </div>
                    </td>
                    <td>
                      <div className="d-flex align-items-center gap-2">
                        ...
                        <div>
                          ...
                          <small className="text-muted">{po.createdBy.role}</small>
                        </div>
                      </div>
                    </td>
                    <td>
                    ...
                    <td>
                      <div className="d-flex align-items-center gap-2">
                        <span className={`fw-semibold ${po.totalValue > 5000 ? 'text-danger' : 'text-success'}`}>
        ...
        <div className="d-flex flex-wrap align-items-center justify-content-between gap-2 mt-24">
          <span>
            Showing {((currentPage - 1) * itemsPerPage) + 1} to {Math.min(currentPage * itemsPerPage, filteredAndSortedPOs.length)} of {filteredAndSortedPOs.length} entries
          </span>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              ...
              <div className="modal-body">
                <p>Are you sure you want to approve <strong>{selectedPO.poNo}</strong> from <strong>{selectedPO.supplier.name}</strong>?</p>
                <p><strong>Total Value:</strong> ${selectedPO.totalValue.toFixed(2)}</p>
                <p>This action will update the PO status to "Approved" and notify the supplier.</p>
              </div>
        ...
        <div className="modal fade show d-block" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              ...
              <div className="modal-body">
                <p>Are you sure you want to reject <strong>{selectedPO.poNo}</strong> from <strong>{selectedPO.supplier.name}</strong>?</p>
                <p><strong>Total Value:</strong> ${selectedPO.totalValue.toFixed(2)}</p>
                ...
                <div className="mb-3">
                  ...
                  <textarea 
                    ...
                    required
                  />
                  <small className="text-muted">{rejectReason.length}/500 characters</small>
                </div>
              </div>
              <div className="modal-footer">
                <button 
        ...
        <div className="modal fade show d-block" style={{ backgroundColor: 'rgba(0,0,0,0.5)' }}>
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content">
              ...
              <div className="modal-body">
                <p>Are you sure you want to reject <strong>{selectedPOs.length}</strong> selected POs?</p>
              ...
              <div className="modal-footer">
                ...
                <button 
                  ...
                  disabled={actionLoading || !rejectReason.trim()}
                >
                  {actionLoading ? 'Rejecting...' : `Reject ${selectedPOs.length} POs`}
                </button>
              </div>
            </div>
          </div>
