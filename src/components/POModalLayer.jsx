...
const POModalLayer = ({
  ...
  const initializeFormForEdit = useCallback((po) => {
    if (po && masterData.suppliers.length > 0) {
      const selectedSupplier = masterData.suppliers.find(s => s.id === po.supplierId);
      setFormState({
        formData: {
          poNo: po.poNo,
  ...
  }, [showModal, masterData.suppliers.length, editingPO, initializeFormForEdit, initializeFormForNew]);
  ...
  useEffect(() => {
    ...
    const hasChanges = updatedLineItems.some((updatedItem, index) =>
  ...
  return (
    <div className={`modal fade ${showModal ? 'show d-block' : ''}`} style={{ backgroundColor: showModal ? 'rgba(0,0,0,0.5)' : 'transparent' }} data-bs-backdrop="static">
      <div className="modal-dialog modal-dialog-centered" style={{ maxWidth: '1400px', width: '95%' }}>
          ...
          <div className="modal-dialog modal-sm modal-dialog-centered">
            <div className="modal-content radius-16 bg-base">
              <div className="modal-body p-24 text-center">
                <div className="mb-16">
                  <Icon icon="mingcute:alert-line" className="text-warning text-4xl" />
                </div>
                <h6 className="text-lg text-neutral-900 mb-8">Unsaved Changes</h6>
