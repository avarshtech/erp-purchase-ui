...
const ItemFormLayer = ({ itemId, onSuccess, onCancel }) => {
  ...
  const handleItemTypeChange = useCallback(
    async (itemTypeId, subCategoryId = formData.subCategoryId) => {
  ...
  // Optimized fetchItem - loads all data in parallel
  const fetchItem = useCallback(async () => {
    try {
      ...
      const [subCatResponse, itemTypesResponse, attributesResponse] =
        await Promise.all([
          getSubcategories(item.categoryId),
          getItemTypes(item.subCategoryId),
          getAttributes(item.subCategoryId, item.itemTypeId),
        ]);
  ...
  const validateForm = () => {
    ...
    attributes.forEach((attr) => {
      if (!attr.is_common && !formData.attributes[attr.id]) {
        newErrors[
          `attribute_${attr.id}`
        ] = `${attr.attribute_name} is required`;
      }
    });
        ...
        <div className="row gy-4">
          ...
          <div className="col-md-6 mb-20">
            ...
            {errors.subCategoryId && (
              <div className="invalid-feedback">{errors.subCategoryId}</div>
            )}
          </div>
          <div className="col-md-6 mb-20">
            <label className="form-label fw-semibold text-primary-light text-sm mb-8">
            ...
            {errors.itemTypeId && (
              <div className="invalid-feedback">{errors.itemTypeId}</div>
            )}
          </div>
          <div className="col-md-6 mb-20">
            <label className="form-label fw-semibold text-primary-light text-sm mb-8">
          ...
          <div className="col-md-6 mb-20">
            ...
            />
            {errors.uomId && (
              <div className="invalid-feedback">{errors.uomId}</div>
            )}
          </div>
          <div className="col-md-6 mb-20">
            <label className="form-label fw-semibold text-primary-light text-sm mb-8">
          ...
          {attributes.length > 0 && (
            <div className="col-12 mb-20">
              <h6 className="fw-semibold text-primary-light text-sm mb-8">
                Attributes
              </h6>
              <div className="row">
                {attributes.map((attr) => (
            ...
            <div className="card bg-light">
              ...
              <div className="card-body">
                ...
                <p>
                  ...
                  {formData.itemName || "Not entered"}
                </p>
                <p>
                  <strong>UOM:</strong> {formData.uomId || "Not entered"}
                </p>
                <p>
                  <strong>HSN Code:</strong> {formData.hsnCode || "Not entered"}
                </p>
                <p>
                  ...
                  <div>
                    ...
                    <ul>
                      {attributes.map((attr) => (
                        <li key={attr.id}>
                          {attr.attribute_name}:{" "}
                          {formData.attributes[attr.id] || "Not entered"}
                        </li>
                      ))}
                    </ul>
                  </div>
